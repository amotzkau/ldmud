...

{
    if (dest->type == T_LVALUE &&
      ...
      || dest->x.lvalue_type == LVALUE_PROTECTED))
    {
        ...
        rover1 = rover2 = dest;
        do {
            if (rover1->type == T_LVALUE && rover1->x.lvalue_type == LVALUE_UNPROTECTED)
                rover1 = rover1->u.lvalue;
            else if (rover1->type == T_LVALUE && rover1->x.lvalue_type == LVALUE_PROTECTED)
                rover1 = &(rover1->u.protected_lvalue->val);
            else
                break;
            if (rover1->type == T_LVALUE && rover1->x.lvalue_type == LVALUE_UNPROTECTED)
                rover1 = rover1->u.lvalue;
                ...
                break;
            if (rover2->type == T_LVALUE && rover2->x.lvalue_type == LVALUE_UNPROTECTED)
                rover2 = rover2->u.lvalue;
            else if (rover2->type == T_LVALUE && rover2->x.lvalue_type == LVALUE_PROTECTED)
                rover2 = &(rover2->u.protected_lvalue->val);
            else
                break;
            if (rover1 == rover2)
            {
        ...
        } while (rover1 && (rover1->type == T_LVALUE)
              && rover2 && (rover2->type == T_LVALUE)
                );
    }
 ...
 */
{
    if (src->type == T_LVALUE)
    {
        for (;;)
        {
            ...
            {
            default:
                fatal("(assign_protected_lvalue_no_free) Illegal lvalue %p type %d\n", src, src->x.lvalue_type);
                /* NOTREACHED */
                break;
...

{
    ...
    case T_POINTER:
      {
        ...
        /* Return the result */
        assign_lvalue_no_free(vec, item);
        return sp;
      }
    ...
    default:
        ...
        inter_sp = sp;
        inter_pc = pc;
        errorf("(push_indexed_lvalue) Indexing on illegal type '%s'.\n", typename(vec->type));
        return sp;
    }
...

{
    ...
    inter_sp = sp;
    inter_pc = pc;
    errorf("(push_rindexed_lvalue) Indexing on illegal type '%s'.\n", typename(vec->type));
    return NULL;
...

{
    ...
    inter_sp = sp;
    inter_pc = pc;
    errorf("(push_aindexed_lvalue) Indexing on illegal type '%s'.\n", typename(vec->type));
    return NULL;
...

{
    ...
    if (vec->type == T_POINTER)
    {
        ...
        arr = vec->u.vec;
        item = get_vector_item(arr, i, sp, pc);

        /* Compute the indexed item and set up the protector. */
        assign_protected_lvalue_no_free(vec, item);
    ...
    inter_sp = sp;
    inter_pc = pc;
    errorf("(push_protected_indexed_lvalue) Indexing on illegal type '%s'.\n", typename(vec->type));
    return NULL;
...

{
    ...
    if (vec->type == T_POINTER)
    {
        ...
        arr = vec->u.vec;
        item = get_vector_r_item(arr, i, sp, pc);
    ...
    inter_sp = sp;
    inter_pc = pc;
    errorf("(push_protected_rindexed_lvalue) Indexing on illegal type '%s'.\n", typename(vec->type));
    return NULL;
...

{
    ...
    if (vec->type == T_POINTER)
    {
        ...
        arr = vec->u.vec;
        item = get_vector_a_item(arr, i, sp, pc);
    ...
    inter_sp = sp;
    inter_pc = pc;
    errorf("(push_protecteded_aindexed_lvalue) Indexing on illegal type '%s'.\n", typename(vec->type));
    return NULL;
...

{
    ...
    inter_sp = sp;
    inter_pc = pc;
    errorf("(push_protected_indexed_map_lvalue) Indexing on illegal type '%s'.\n", typename(vec->type));
    return NULL;
} /* push_protected_indexed_map_lvalue() */
...

{
    ...
    default:
        inter_sp = sp;
        inter_pc = pc;
        errorf("(rindex_lvalue) Indexing on illegal type '%s'.\n", typename(vec->type));
        return NULL;
    }
...

{
    ...
    default:
        ...
        inter_sp = sp;
        inter_pc = pc;
        errorf("(aindex_lvalue) Indexing on illegal type '%s'.\n", typename(vec->type));
        return NULL;
    }
...

{
    ...
    for (;;)
    {
        ...
        case T_POINTER:
          {
            ...
            assign_protected_lvalue_no_free(sp, item);
        ...
        case T_LVALUE:
          {
            switch (vec->x.lvalue_type)
            {
               ...
               default:
                   inter_sp = sp;
                   inter_pc = pc;
                   errorf("(protected_index_lvalue) Indexing on illegal lvalue type %d.\n"
                         , vec->x.lvalue_type);
                   return NULL;
            }
          }
        ...
        default:
            ...
            inter_sp = sp;
            inter_pc = pc;
            errorf("(protected_index_lvalue) Indexing on illegal type '%s'.\n", typename(vec->type));
            return NULL;
        } /* switch() */
    } /* for(ever) */
...

{
    ...
    for (;;)
    {
        ...
        case T_LVALUE:
          {
            switch (vec->x.lvalue_type)
            {
               ...
               default:
                   inter_sp = sp;
                   inter_pc = pc;
                   errorf("(protected_rindex_lvalue) Indexing on illegal lvalue type %d.\n"
                         , vec->x.lvalue_type);
                   return NULL;
            }
          }
        ...
        default:
            ...
            inter_sp = sp;
            inter_pc = pc;
            errorf("(protected_rindex_lvalue) Indexing on illegal type '%s'.\n", typename(vec->type));
            return NULL;
        }
    } /* for(ever) */
...

{
    ...
    for (;;)
    {
        ...
        case T_LVALUE:
          {
            switch (vec->x.lvalue_type)
            {
               ...
               default:
                   inter_sp = sp;
                   inter_pc = pc;
                   errorf("(protected_aindex_lvalue) Indexing on illegal lvalue type %d.\n"
                         , vec->x.lvalue_type);
                   return NULL;
            }
          }
        ...
        default:
            ...
            inter_sp = sp;
            inter_pc = pc;
            errorf("(protected_aindex_lvalue) Indexing on illegal type '%s'.\n", typename(vec->type));
            return NULL;
        }
    } /* for(ever) */
...

{
    ...
    default:
        inter_sp = sp;
        errorf("(range_lvalue) Range index on illegal type '%s'.\n", typename(vec->type));
        return NULL;
    }
...

{
    ...
    default:
        inter_sp = sp;
        errorf("(protected_range_lvalue) Range index on illegal type '%s'.\n", typename(vec->type));
        return NULL;
    }
...

{
    ...
    default:
        inter_sp = sp;
        inter_pc = pc;
        errorf("(push_rindexed_lvalue) Range index on illegal type '%s'.\n", typename(vec->type));
        return NULL;
    }
...

{
    ...
    default:
        inter_sp = sp;
        inter_pc = pc;
        errorf("(push_aindexed_lvalue) Range index on illegal type '%s'.\n", typename(vec->type));
        return NULL;
    }
    ...
    if ((i = csp->num_local_variables - num_arg) > 0)
    {
        ...
        if (num_arg < 0)
        {
            ...
            else
            {
                ...
                while (--i >= 0)
                {
                    if (!allowRefs && sp->type == T_LVALUE && sp->x.lvalue_type == LVALUE_UNPROTECTED)
...
#   endif
    ...
    CASE(F_VOID_ADD_EQ);            /* --- void_add_eq         --- */
    {
        ...
        for (;;)
        {
            ...
            case T_LVALUE: /* Unravel lvalue. */
                ...
                case LVALUE_PROTECTED:
                    argp = &(argp->u.protected_lvalue->val);
                    continue;
                } /* switch() */
                break;
    ...
    CASE(F_PUSH_INDEXED_MAP_LVALUE); /* --- push_indexed_map_lvalue --- */
    {
        ...
        if (sp[-2].type != T_MAPPING)
        {
            ERRORF(("(push_indexed_map_lvalue) Indexing on illegal type: %s, expected mapping.\n"
                   , typename(sp[-2].type)
                  ));
        }
        if (sp[0].type != T_NUMBER)
    ...
    CASE(F_FOREACH_RANGE); /* --- foreach_range <nargs> <offset> --- */
    {
         ...
         * to loop over.
         */
        if (gen_refs && (sp->type != T_LVALUE || sp->x.lvalue_type != LVALUE_UNPROTECTED))
        {
        ...
        for (arg = sp
            ; gen_refs && arg && arg->type == T_LVALUE && arg->x.lvalue_type == LVALUE_UNPROTECTED
            ; arg = arg->u.lvalue)
            NOOP;
        ...
        else
        {
            ...
            else if (sp[-2].type == T_POINTER)
            {
                ...
                else
                {
                    ...
                    /* Compute the indexed item and create the lvalue */
                    item = &vec->u.vec->item[ix];
                    assign_protected_lvalue_no_free(lvalue, item);
                }
            }
#ifdef USE_STRUCTS
    ...
    CASE(F_REFERENCEP);                /* --- referencep      --- */
      {
        ...
        i = (sp->type == T_LVALUE && sp->x.lvalue_type == LVALUE_UNPROTECTED
          && sp->u.lvalue->type == T_LVALUE);
        free_svalue(sp);
        put_number(sp, i);
        break;
      }
...

{
    ...
    if (b_use_default)
    {
        ...
        if (hook->type == T_STRING || hook->type == T_CLOSURE)
        {
             ...
             */
            assign_lvalue_no_free(argp, &result);
