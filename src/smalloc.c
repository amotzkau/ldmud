    ...
    for (p = heap_start; p < last; )
    {
        ...
        if ( (flags & (M_REF|THIS_BLOCK|M_GC_FREE)) == (THIS_BLOCK|M_GC_FREE) )
        {
            ...
            dprintf2(gcollect_outfd, " %s size 0x%x\n",
              p[XM_SRC_LOCATION+ML_OVERHEAD], size & M_MASK
            );
#endif
#ifdef MALLOC_LPC_TRACE
...
#ifdef MALLOC_TRACE
                dprintf1(gcollect_outfd, " %s", q[XM_SRC_LOCATION+M_OVERHEAD]);
#endif
    ...
    for (p = last_small_chunk; p; p = *(word_t**)p)
    {
        ...
        for (q = p+1; q < end; )
        {
            ...
            if (!(*q & THIS_BLOCK))
            {
                ...
                {
#ifdef MALLOC_TRACE
                    if (q[XM_SRC_LOCATION+M_OVERHEAD])
                        dprintf1(fd, ": %s ", q[XM_SRC_LOCATION+M_OVERHEAD]);
                    else
                        writes(fd, ": - - ");
#endif
#ifdef MALLOC_LPC_TRACE
